...
export default function Sidebar() {
    ...
    return (
        ...
        `}
            >
                {/* Header */}
                <div className="h-16 flex items-center px-4 border-b border-slate-100 dark:border-slate-800 justify-between overflow-hidden">
                ...
                {/* Nav */}
                <div className="flex-1 overflow-y-auto py-6 px-4 lg:px-3 space-y-6 lg:space-y-4 custom-scrollbar">
                    {menuGroups.map((group, idx) => (
                        <div key={idx}>
                            {!isCollapsed && (
                                <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-3">
                                    {group.label}
                                </div>
                            )}
                                ...
                                {group.items.map((item, i) => {
                                    ...
                                    return (
                                        <Link
                                            ...
                                            href={targetPath}
                                            onClick={() => setIsMobileOpen(false)}
                                            className={`flex items-center group relative px-3 py-3 lg:py-2.5 rounded-lg transition-all ${isActive
                                                ? "bg-[#1565C0]/10 text-[#1565C0]"
                                                : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"
                                                }`}
                                        >
                                            <span
                                                className={`material-icons-outlined text-2xl lg:text-xl flex-shrink-0 ${isCollapsed && !isMobileOpen ? "mx-auto" : "mr-3"
                                                    }`}
                                            >
                                                {item.icon}
                                            </span>
                                            {(!isCollapsed || isMobileOpen) && (
                                                <span className="font-medium whitespace-nowrap text-lg lg:text-base">
                                                    {item.label}
                                                </span>
                                            )}
                                            {isCollapsed && !isMobileOpen && (
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800">
                    <Link
                        href="/"
                        className="w-full flex items-center justify-center px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-100 transition-colors"
                    >
                        <span className="material-icons-round text-sm">logout</span>
                        {!isCollapsed && (
                            <span className="ml-2 font-medium">登出</span>
                        )}
                    </Link>
                </div>
            </aside>
