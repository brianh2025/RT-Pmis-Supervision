...
import {
  ...
  CheckCircle2, AlertTriangle, Clock, Send, FileText, ExternalLink,
  ChevronLeft, ChevronRight, Sun, Moon, LogOut,
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useTheme } from '../context/ThemeContext';
import { supabase, PMIS_BASE_URL } from '../lib/supabaseClient';
...
const fmtMonthLabel = (y, m) => `${y - 1911} 年 ${m + 1} 月`;

/* 西元 YYYY-MM-DD → 民國 115/10/05 */
function fmtRoc(iso) {
  if (!iso) return '—';
...
const diaryUrl = (projectId) => `${PMIS_BASE_URL}/projects/${projectId}/supervision`;
...
export function ReportCheck() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      setLoadError('');
      const [projRes, logRes, repRes] = await Promise.all([
      ...
      const failed = [projRes, logRes, repRes].find(r => r.error);
      if (failed) {
        setLoadError(`資料載入失敗：${failed.error.message}`);
        setLoading(false);
        return;
      }

      const sorted = (projRes.data || []).slice().sort((a, b) => {
      ...
      setProjects(sorted);
      setLogSet(new Set((logRes.data || []).map(r => `${r.project_id}|${r.log_date}`)));
      const map = {};
      (repRes.data || []).forEach(r => { map[`${r.project_id}|${r.report_month}`] = r; });
      setReports(map);
      setLoading(false);
    }
    load();
  ...
  const cellStat = useCallback((project, y, m) => {
    ...
    const mEnd = dateStr(y, m, daysInMonth(y, m));

    if (!project.start_date) return { active: false };
    const start = project.start_date > mStart ? project.start_date : mStart;
    let end = mEnd;
    ...
    return {
      ...
      logState: missing.length === 0 ? 'full' : missing.length >= expected ? 'empty' : 'partial',
    };
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter(p =>
      p.name.toLowerCase().includes(q) || (p.contractor || '').toLowerCase().includes(q));
  }, [projects, query]);
  ...
  /* 年度總表：勾選「僅顯示未完成」時，隱藏全年皆已發文且日誌完整的工程 */
    ...
    <div className="rc-app">
      <header className="rc-header">
        ...
        <div className="rc-title-block">
          <h1 className="rc-title">提送管制</h1>
          <span className="rc-subtitle">跨工程檢核每月施工日誌建檔與監造月報提送、發文狀態</span>
        </div>
        ...
        <div className="rc-account">
          <button className="rc-icon-btn" onClick={toggleTheme} title={isDarkMode ? '切換亮色' : '切換暗色'}>
            {isDarkMode ? <Sun size={16} /> : <Moon size={16} />}
          </button>
          <span className="rc-user" title={user?.email}>{user?.email}</span>
          <button className="rc-icon-btn" onClick={signOut} title="登出">
            <LogOut size={16} />
          </button>
        </div>
      </header>
      ...
      {loadError ? (
        <div className="rc-error">{loadError}</div>
      ) : loading ? (
        ...
        monthRows.length === 0 ? (
          ...
          <div className="rc-table-wrap thin-scrollbar">
                ...
                {monthRows.map(({ project, stat }) => (
                  <tr key={project.id} className="list-item-enter">
                    ...
                    <td className="rc-col-action">
                      ...
                      <a className="rc-row-btn" href={diaryUrl(project.id)} target="_blank" rel="noreferrer">
                        <ExternalLink size={14} />日誌
                      </a>
                    </td>
                  </tr>
                ))}
          ...
          <table className="rc-matrix">
            <thead>
              <tr>
                ...
                {MONTHS.map((label, m) => (
                  <th key={label} className={m === today.getMonth() && year === today.getFullYear() ? 'rc-matrix-current' : ''}>{label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                  ...
                  {MONTHS.map((label, m) => {
                    ...
                    return (
                      <td
                        ...
                        title={`${project.name} ${fmtMonthLabel(year, m)}`}
                      >
      ...
      {detail && (
        <div className="rc-modal-overlay" onClick={e => e.target === e.currentTarget && setDetail(null)}>
          <div className="rc-modal animate-slide-up">
            ...
            <div className="rc-modal-body thin-scrollbar">
              {(() => {
                const stat = cellStat(detail.project, detail.y, detail.m);
                return (
                  <>
                    <div className="rc-section">
                      <div className="rc-section-title"><span className="rc-accent" />施工日誌建檔</div>
                      <div className="rc-diary-summary">
                        {logBadge(stat)}
                        <span className="rc-muted">工期內應建檔 {stat.expected} 日，已建檔 {stat.logged} 日</span>
                      </div>
                      {stat.missing.length > 0 && (
                        <div className="rc-missing-list thin-scrollbar">
                          {stat.missing.map(d => <span key={d} className="rc-missing-chip">{d.slice(5).replace('-', '/')}</span>)}
                        </div>
                      )}
                      <a className="rc-link-btn" href={diaryUrl(detail.project.id)} target="_blank" rel="noreferrer">
                        <ExternalLink size={14} />到 RT-PMIS 開啟該工程日誌
                      </a>
                    </div>

                    <div className="rc-section">
                      <div className="rc-section-title"><span className="rc-accent" />監造月報提送 / 發文</div>
                      ...
                      {saveError && <div className="rc-error">{saveError}</div>}
                    </div>
                  </>
                );
              })()}
