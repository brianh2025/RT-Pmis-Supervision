...
## 7. 版面慣例（2026-07-13 確立）

- **區塊標題標準樣式**：前方束線（3px × 18px、`--color-primary`、圓角 2px）＋標題文字 `--fs-base` / 600 / `--color-text1`，flex 同列、gap 8px（見進度管理「進度曲線」「歷史進度紀錄」標題）
- **sticky 頂列毛玻璃**：捲動容器內固定標題列用 `position: sticky; top: 0` ＋ `--color-surface-translucent` 底 ＋ `backdrop-filter: blur(12px)` ＋ 底框線（見總覽頁標題列）
- **輪播訊息行數**：桌機單行 ellipsis；行動版（≤768px）`-webkit-line-clamp: 2` 最多兩行截斷，容器 `min-height` 自動長高
- **總覽頁行動版頂部**：頂列（Topbar）＝標題＋說明＋明暗切換；第二列＝漢堡鍵＋搜尋列＋動作按鈕同列
